...
async function startSCRCPY() {
  ...
  exec(scrcpyCmd, (error, stdout, stderr) => {
    if (error) {
      ...
      win.webContents.send("cmd_sent", { success: error });
      return;
    }

    if (stderr) {
      console.error("scrcpy stderr:", stderr);
      // win.webContents.send('cmd_sent', { success: stderr });
      return;
    }
...
async function restoreAppPrefs(packageName, removeAfter = true) {
  ...
  return adbShell(`run-as ${packageName} ${cmd} "${backupPath}" "/data/data/"`);
}
...
async function initLogs() {
  ...
  if (await fsp.exists(logPath)) {
    await fsp.unlink(logPath);
  } else {
    await fsp.mkdir(global.sidenoderHome, { recursive: true });
  }
  ...
  console.log = (...d) => {
    ...
    for (const l of d) {
      ...
      const formatted = util.format(l);
      line += `${formatted} `;
      lineColor += chalk.green(`${formatted} `);
    }

    logStdout.write(`${dateF()}${lineColor}\n`);
    logFile.write(`${dateF()}${line}\n`);
  };
  ...
  console.error = (...d) => {
    ...
    logStdout.write(chalk.red(`${dateF()}ERROR: ${line}\n`));
    logFile.write(`${dateF()}ERROR: ${line}`);
  };
  ...
  console.warning = (...d) => {
    ...
    logStdout.write(chalk.yellow(`${dateF()}WARN: ${line}\n`));
    logFile.write(`${dateF()}WARN: ${line}\n`);
  };
}
